...
router.get("/suggested_accounts", verifyUserToken, async(req, res) => {
    try {
    const user = req.authorized_account;
    let following = user.following.sort(() => Math.random() - Math.random()).slice(0, 6);
    ...
    if(suggestions.length < 3 || following.length < 1) {
        const FindRandomUsersToSugges = await users.find({censored: false || undefined})
        const filterUsers = FindRandomUsersToSugges.filter(tsd => !user.following.includes(tsd._id))
       
        const SortUsersToSuggest = filterUsers.sort((a,b) => (a.followers.length < b.followers.length ? 1 : -1));
        const FindRandomUsersToSuggest = SortUsersToSuggest.slice(0, 6-suggestions.length)
